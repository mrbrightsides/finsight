    ...
    <div className="space-y-10 animate-fadeIn pb-12 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
        ...
        <form onSubmit={handleSubmit} className="lg:col-span-3 space-y-8">
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
             ...
             <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                  ...
                  <div className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">Derived from Revenue</div>
                </div>
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                  ...
                  <div className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">Derived from Budget</div>
                </div>
                <div className="bg-indigo-50 p-6 rounded-3xl border border-indigo-100">
                  <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-2 block">Monthly Surplus</label>
                  <div className={`text-2xl font-black ${formData.monthlyIncome - formData.monthlyExpenses >= 0 ? 'text-indigo-600' : 'text-rose-600'}`}>
                    ${(formData.monthlyIncome - formData.monthlyExpenses).toLocaleString()}
                  </div>
                  <div className="text-[9px] font-bold text-indigo-300 mt-1 uppercase tracking-tighter">Net Cash Flow</div>
                </div>
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Target Savings ($)</label>
                  <input 
                    type="number" 
                    value={formData.monthlySavings || 0} 
                  ...
                  />
                  <div className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">Manual Strategic Goal</div>
                </div>
             </div>
          </div>
          ...
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
             <div className="flex justify-between items-center border-b border-slate-50 pb-6">
                <h3 className="text-xl font-black text-slate-900">Strategic Budgeting</h3>
                <button type="button" onClick={handleAddBudget} className="bg-rose-50 text-rose-600 px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-100 transition-all">+ Add Category</button>
             </div>
             <div className="space-y-4">
                {formData.budgets.map((budget) => (
                  <div key={budget.id} className="grid grid-cols-1 md:grid-cols-4 gap-4 p-6 bg-rose-50/30 rounded-3xl border border-rose-100/50">
                    <div className="md:col-span-1"><label className="text-[9px] font-black text-rose-600 uppercase mb-1 block">Category</label><input value={budget.name} onChange={(e) => handleUpdateBudget(budget.id, 'name', e.target.value)} className="w-full px-4 py-2 rounded-xl border-2 border-rose-100 text-sm font-bold focus:border-rose-500 outline-none bg-white" /></div>
                    <div><label className="text-[9px] font-black text-rose-600 uppercase mb-1 block">Target ($)</label><input type="number" value={budget.target} onChange={(e) => handleUpdateBudget(budget.id, 'target', Number(e.target.value))} className="w-full px-4 py-2 rounded-xl border-2 border-rose-100 text-sm font-bold focus:border-rose-500 outline-none bg-white" /></div>
                    <div><label className="text-[9px] font-black text-rose-600 uppercase mb-1 block">Actual Spend ($)</label><input type="number" value={budget.actual} onChange={(e) => handleUpdateBudget(budget.id, 'actual', Number(e.target.value))} className="w-full px-4 py-2 rounded-xl border-2 border-rose-100 text-sm font-bold focus:border-rose-500 outline-none bg-white" /></div>
                    <div className="flex items-end justify-end"><button type="button" onClick={() => handleRemoveBudget(budget.id)} className="p-3 text-rose-300 hover:text-rose-500 transition-colors"><i className="fas fa-trash-can"></i></button></div>
                  </div>
                ))}
             </div>
          </div>
