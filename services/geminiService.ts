...
const getAI = () => new GoogleGenAI({ apiKey: import.meta.env.GEMINI_API_KEY });
...
export const analyzeBondStrategy = async (profile: UserProfile, bondData: any) => {
  return callWithRetry(async () => {
    const ai = getAI();
    const prompt = `Act as a Fixed Income Strategist.
...
export const analyzeTaxImpact = async (profile: UserProfile) => {
  return callWithRetry(async () => {
    ...
    const prompt = `Act as an educational Tax Architect. 
...
export const analyzeDeFiStrategy = async (profile: UserProfile) => {
  return callWithRetry(async () => {
    ...
    const prompt = `Act as a DeFi (Decentralized Finance) Protocol Specialist.
...
export const getRebalanceSuggestions = async (profile: UserProfile) => {
  return callWithRetry(async () => {
    ...
    const prompt = `Act as a Senior Wealth Management Architect.
...
export const createAdvisorChat = (profile: UserProfile) => {
  ...
  return ai.chats.create({
    model: 'gemini-3-flash-preview',
    config: { systemInstruction: `You are 'Finny', lead AI Advisor. Help with financial literacy. User: ${profile.name}, NW: ${profile.totalBalance}.` },
  });
};
